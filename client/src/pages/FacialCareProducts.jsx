...
const FACIAL_CATEGORIES = [
  ...
  { id: 'Toner', name: 'Step 2: Tone 🧪', desc: 'Balance skin pH and prep for deep absorption' },
  { id: 'Serum', name: 'Step 3: Treat (Serums) 💧', desc: 'Target active concerns with high-concentration actives' },
  { id: 'Moisturiser', name: 'Step 4: Hydrate 🧴', desc: 'Seal in moisture and strengthen the skin barrier' },
  { id: 'Sunscreen', name: 'Step 5: Protect ☀️', desc: 'Defend against UV damage and premature ageing' },
  { id: 'Mask', name: 'Weekly Boost: Masks 🌸', desc: 'Specialized intensive treatment for your skin goals' }
]
...
function FacialCareProducts() {
  ...
  const fetchWeatherAndRecommendations = useCallback(async (profile) => {
    ...
    try {
      ...
      // Filter for facial care products only
      const facialCareList = productsRes.data.products.filter(p =>
        ['Cleanser', 'Toner', 'Serum', 'Moisturiser', 'Sunscreen', 'Mask'].includes(p.category)
      )
      setProducts(facialCareList)
      ...
      try {
        ...
        setMamaTip('Apply fresh aloe vera mixed with rose water to soothe your skin today.')
      } finally {
        setMamaTipLoading(false)
      }
    } catch (err) {
      console.error('Error fetching facial care recommendations:', err)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const getPersonalizedExplanation = (product) => {
    ...
    // 1. Check skin type match
    const suitsSkin = product.suitability?.skinTypes?.includes('all') || product.suitability?.skinTypes?.includes(facialProfile.skinType)
    if (suitsSkin) {
      explanationPoints.push(
        <li key="skin-type">
          ✨ Perfect for your <strong>{facialProfile.skinType}</strong> skin type (formulated to balance and support your natural skin barrier).
        </li>
      )
    }

    // 2. Check skin concern match
    ...
    // 3. Highlight matching ingredients
    const matchedIngredients = product.ingredients?.filter(ing =>
      ing.suitableFor?.includes('all') || ing.suitableFor?.includes(facialProfile.skinType)
    ) || []

    if (matchedIngredients.length > 0) {
      explanationPoints.push(
        <li key="ingredients">
          🔬 Contains <strong>{matchedIngredients[0].name}</strong>: {matchedIngredients[0].benefit}.
        </li>
      )
    }
    ...
    return (
      <ul className="personalized-explanation-list">
        {explanationPoints.length > 0 ? explanationPoints : (
          <li key="fallback">🌿 Dermatologist recommended, gentle daily care formula.</li>
        )}
      </ul>
    )
  }
  ...
  return (
    <div className="facial-products-page">
      {/* Header navbar */}
      <header className="facial-products-header">
      ...
      {/* Hero skin profile banner */}
      <section className="skin-profile-banner">
        <div className="banner-left">
          <span className="badge-profile">YOUR CUSTOM PROFILE</span>
          <h2>Personalised Skincare Recommendations</h2>
          <p>
            We've analysed your profile: <strong>{facialProfile.skinType.toUpperCase()}</strong> Skin 
            {facialProfile.skinConcerns.length > 0 && (
              <>
                {' '}targeting <strong>{facialProfile.skinConcerns.join(', ')}</strong>
              </>
            )}
            {weather && (
              <>
        ...
        <div className="mama-tip-corner-card">
          <div className="mama-tip-header">
            ...
            <div className="mama-tip-title-box">
              ...
              <p className="mama-tip-subtitle">AI-generated traditional care for your {facialProfile.skinType} skin</p>
            </div>
          </div>
      ...
      <div className="category-tabs-container">
        ...
        <div className="active-tab-desc">
          <p>{FACIAL_CATEGORIES.find(c => c.id === activeTab)?.desc}</p>
        </div>
      </div>

      {/* Products Grid */}
      <main className="facial-products-main">
        {loading ? (
          <div className="facial-loading">
            <span className="loading-spinner">🌿</span>
            <p>Tailoring matching formulas to your skin type...</p>
          </div>
        ) : filteredProducts.length === 0 ? (
          <div className="facial-no-products">
            <p className="no-p-emoji">🔍</p>
            <h4>No products found in this category</h4>
            <p>Try updating your skin profile or concerns to see recommended items.</p>
          </div>
        ) : (
            ...
            {filteredProducts.map(product => (
              ...
              >
                <div className="card-image-wrap">
                  ...
                  {product.score >= 6 && <span className="premium-badge">Highly Recommended</span>}
                </div>
                ...
                <div className="card-body">
                  ...
                  <div className="personalized-explanation-box">
                    <h4 className="explanation-title">🌿 Why it suits your skin:</h4>
...
export default FacialCareProducts
